import { ActivityLevel, GoalType, UserProfile } from './profile';
...
export function mifflinStJeorBmr(sex: UserProfile['sex'], kg: number, cm: number, age: number): number {
  const base = 10 * kg + 6.25 * cm - 5 * age;
  return sex === 'male' ? base + 5 : base - 161;
}

export function bmiOf(kg: number, cm: number): number {
  const m = cm / 100;
  if (m <= 0) return 0;
  return kg / (m * m);
}
...
export function proteinReferenceKg(weightKg: number, heightCm: number): number {
...
function proteinPerKg(goal: GoalType): number {
  switch (goal) {
    case 'lose_weight':
...
function calorieAdjustment(goal: GoalType, tdee: number): number {
  switch (goal) {
    case 'lose_weight':
...
function stepGoalFor(goal: GoalType): number {
  switch (goal) {
    case 'lose_weight':
      return 10000;
    case 'run_5k':
...
export function computeTargets(profile: UserProfile): NutritionTargets {
