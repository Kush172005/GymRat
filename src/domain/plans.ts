import { Experience, GoalType } from './profile';
...
function takeDays(pool: PlanDay[], n: number): PlanDay[] {
  const out: PlanDay[] = [];
  for (let i = 0; i < n; i++) out.push(pool[i % pool.length]);
  return out;
}

export function planForGoal(goal: GoalType, daysPerWeek: number, experience: Experience): TrainingPlan {
...
const TRAINING_DOWS: Record<number, number[]> = {
  2: [1, 4],
  3: [1, 3, 5],
  4: [1, 2, 4, 5],
  5: [1, 2, 3, 4, 5],
...
export function isTrainingDay(daysPerWeek: number, date = new Date()): boolean {
  return (TRAINING_DOWS[daysPerWeek] ?? TRAINING_DOWS[3]).includes(date.getDay());
}
