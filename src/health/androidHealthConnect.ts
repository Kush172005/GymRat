...
async function ensureSdk(): Promise<HealthStatus> {
...
function hasRead(granted: Permission[], type: RecordType): boolean {
  return granted.some((p) => p.accessType === 'read' && p.recordType === type);
}

async function currentStatus(): Promise<HealthStatus> {
...
export const androidHealthConnect: HealthProvider = {
  id: 'health_connect',
