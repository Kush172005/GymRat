...
import { RootStackParamList, YouStackParamList } from '../../navigation/types';
...
export function ProfileSetupScreen({ navigation }: { navigation: Nav }) {
  const { colors } = useTheme();
  const units = settingsRepo.getUnits();
  const existing = profileRepo.get();
  const [step, setStep] = useState(0);
  const [p, setP] = useState<UserProfile>(existing ?? empty);
        ...
        <ScrollView contentContainerStyle={styles.pad} keyboardShouldPersistTaps="handled">
          ...
          {step === 2 && (
            <>
              ...
              <View style={styles.wrap}>
                {ACTIVITY_OPTIONS.map((a) => (
                  <Chip
                    ...
                    onPress={() => setP({ ...p, activity: a.id as ActivityLevel })}
                    style={styles.chip}
                  />
                ))}
              </View>
              ...
              <View style={styles.wrap}>
                {(['beginner', 'intermediate'] as Experience[]).map((e) => (
...
function Field({
  label, value, onChange, keyboard, colors,
}: {
  label: string;
  value: string;
