...
import { workoutRepo, FullWorkoutSession } from '../../db/workoutRepo';
...
type Props = YouStackScreenProps<'WorkoutHistory'>;

export function WorkoutHistoryScreen({ navigation }: Props) {
  const { colors } = useTheme();
  const units = settingsRepo.getUnits();
  const [reloadKey, setReloadKey] = useState(0);
  const [expanded, setExpanded] = useState<Record<string, FullWorkoutSession | null>>({});
...
function GridStat({
  ...
  label: string;
  value: string;
  colors: { primary: string; text: string; textMuted: string };
}) {
  return (
