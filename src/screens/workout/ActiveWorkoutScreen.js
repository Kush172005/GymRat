...
export function ActiveWorkoutScreen({ route, navigation }) {
  ...
  const removeExercise = (weId, name) => {
...
function progressCopy(pr, hasEntry, weightKg, reps, units) {
  if (!pr) {
...
function ExerciseLogRow({ workoutExercise, units, autoFocus, onRemove, onChange }) {
  const { colors } = useTheme();
